...
class DashboardOperationViewControllerAcc: UIViewController {
    ...
    private let gRateValueView: SelectorValueView
    private let gRateSelector: SelectorSlider
    ...
    init(viewModel: DashboardContainerViewModel) {
        ...
        gRateSelector.setMarkerImages(markedImage: UIImage(named: "image_slider_marker_marked.pdf"),
                                      unmarkedImage: UIImage(named: "image_slider_marker_unmarked.pdf"))
        gRateSelector.setMinimumTrackImage(UIImage(named: "image_slider_line_marked.pdf"), for: .normal)
        gRateSelector.setMaximumTrackImage(UIImage(named: "image_slider_line_unmarked.pdf"), for: .normal)
        gRateSelector.setThumbImage(UIImage(named: "icon_slider_knob.pdf"), for: .normal)
        gRateSelector.delegate = self
    }
    ...
    @objc private func displayLinkTick() {
        guard let displayLink = displayLink,
            ...
            NSLog("DashboardOperationViewControllerAcc::scrollTick frame duration error.")
            return
        }
    ...
    private func plotDimensions(_ dimensions: [(CGFloat, UIColor)], step: CGFloat, scale: CGFloat) {
        guard let currentSlice = currentSlice,
            ...
            NSLog("DashboardOperationViewControllerAcc::plotVector integrity error.")
            return
        }
    ...
    private func layoutView() {
        ...
        selectorContainer.addSubview(gRateValueView)
        selectorContainer.addSubview(gRateSelector)
        ...
        gRateValueView.translatesAutoresizingMaskIntoConstraints = false
        gRateSelector.translatesAutoresizingMaskIntoConstraints = false
        ...
        NSLayoutConstraint.activate(
            [gRateValueView.leadingAnchor.constraint(equalTo: selectorContainer.leadingAnchor),
             gRateValueView.trailingAnchor.constraint(equalTo: selectorContainer.trailingAnchor)])

        NSLayoutConstraint.activate(
            [gRateSelector.leadingAnchor.constraint(equalTo: selectorContainer.leadingAnchor),
             gRateSelector.trailingAnchor.constraint(equalTo: selectorContainer.trailingAnchor),
             gRateSelector.bottomAnchor.constraint(equalTo: selectorContainer.bottomAnchor)])
        ...
        NSLayoutConstraint.activate(
            [sampleRateSelector.topAnchor.constraint(equalTo: sampleRateValueView.bottomAnchor, constant: 6.0).with(.defaultHigh),
             gRateValueView.topAnchor.constraint(equalTo: sampleRateSelector.bottomAnchor, constant: 16.0).with(.defaultHigh),
             gRateSelector.topAnchor.constraint(equalTo: gRateValueView.bottomAnchor, constant: 6.0).with(.defaultHigh)])

        view.layoutIfNeeded()
    }
}

extension DashboardOperationViewControllerAcc: SelectorSliderDelegate {
    ...
    func valueSelected(sender: UIControl, value: NSNumber) {
        ...
        case gRateSelector:
            guard let (index, _) = (viewModel.resourceParameters(resource: .accConfig).enumerated()
                .first { $0.element.value == value.stringValue }) else { return }

            viewModel.requestSend(resource: .accConfig, method: .put, parameterIndices: [index])
            gRateValueView.value = value
...
extension DashboardOperationViewControllerAcc: Observer {
